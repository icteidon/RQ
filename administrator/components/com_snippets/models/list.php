...
class SnippetsModelList extends JModelList
{
	...
	function import($model)
	{
		...
		if (!is_array($file) || !isset($file['name'])) {
			...
			$app->redirect('index.php?option=com_snippets&view=list&layout=import', $msg);
		}
		...
		$app->redirect('index.php?option=com_snippets&view=list', $msg);
	}
	...
	{
		$db = $this->getDbo();
		$sql = "SELECT * FROM #__snippets WHERE id IN ( ".implode(',', $ids)." )";
		$db->setQuery($sql);
