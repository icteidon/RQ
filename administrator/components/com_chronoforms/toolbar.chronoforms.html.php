...
class menuChronoForms {
	...
	function form_wizard_menu(){
		$formname = "New Form...";
		if(isset($_GET['formname']) && !empty($_GET['formname'])){
			$formname = $_GET['formname'];
		}
		JToolBarHelper::title(JText::_('Form Wizard').": ".$formname);
		JToolBarHelper::custom($task = 'preview', $icon = 'preview.png', $iconOver = 'preview.png', $alt = 'Test Form', $listSelect = false);
		JToolBarHelper::custom($task = 'fullscreen', $icon = 'fullscreen_b.png', $iconOver = 'fullscreen_b.png', $alt = 'Full Screen', $listSelect = false);
	...
	function cancel_menu(){
		...
		if($task == 'install_action'){
			...
			JToolBarHelper::title(JText::_('Updates'));
		}
