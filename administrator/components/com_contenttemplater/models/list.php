...
class ContentTemplaterModelList extends JModelList
{
	...
	function import($model)
	{
		...
		if (!is_array($file) || !isset($file['name'])) {
			...
			$app->redirect('index.php?option=com_contenttemplater&view=list&layout=import', $msg);
		}
		...
		$app->redirect('index.php?option=com_contenttemplater&view=list', $msg);
	}
	...
	{
		$db = $this->getDbo();
		$sql = "SELECT * FROM #__contenttemplater WHERE id IN ( ".implode(',', $ids)." )";
		$db->setQuery($sql);
