...
class plgSystemArticlesAnywhereHelper
{
		...
		if (preg_match($regex_close, $string)) {
			...
			while ($count++ < 10 && preg_match($regex_close, $string) && preg_match_all($regex, $string, $matches, PREG_SET_ORDER) > 0) {
				foreach ($matches as $match) {
						$html = $this->processMatch($string, $art, $match, $message);
	...
	function processMatch(&$string, &$art, &$match, &$message)
	{
		$html = '';
		if ($message != '') {
			...
			$html = $this->processArticle($id, $art, $html, $type);
	...
	function processArticle($article, $art, $text = '', $type = 'article', $first = 0)
	{
		if (!$first) {
			// do second pass
			$text = $this->processArticle($article, $art, $text, $type, 1);
		}
		...
		$where = ' AND ( a.title = '.$this->database->quote(NNFrameworkFunctions::html_entity_decoder($article));
		$where .= ' OR a.alias = '.$this->database->quote(NNFrameworkFunctions::html_entity_decoder($article));
		if (is_numeric($article)) {
			$where .= ' OR a.id = '.$article;
		}
		$where .= ' ) ';
		if (!$this->params->ignore_state) {
			...
			$where .= ' AND ( a.publish_up = '.$this->database->quote($nullDate).' OR a.publish_up <= '.$this->database->quote($now).' )'
				.' AND ( a.publish_down = '.$this->database->quote($nullDate).' OR a.publish_down >= '.$this->database->quote($now).' )';
		}
		...
		$query = 'SELECT '.$selects
			.' FROM #__'.$from.' as a'
			.$joins
			.' WHERE a.access IN ('.implode(',', $this->params->aid).')'
			.$where
			.' ORDER BY a.ordering'
			.' LIMIT 1';
	...
	{
		NNFrameworkFunctions::protectForm($str, '{'.$this->params->article_tag);
	}

	function unprotect(&$str)
	{
		NNFrameworkFunctions::unprotectForm($str, '{'.$this->params->article_tag);
	}
