...
class plgSystemTabberHelper
{
	...
	function onAfterDispatch()
	{
		...
		if ($docType !== 'feed' && !JRequest::getInt('print')) {
			...
			$css_i = 'li.'.$this->alias.'_'.$this->item;
			if ($this->params->rounded && $this->params->rounded_radius && (int) $this->params->rounded_radius != 10) {
				...
				$style .= '
					...
					'.$css_c.'.rounded div.'.$this->alias.'_nav '.$css_i.' a,
					'.$css_c.'.rounded div.'.$this->alias.'_nav '.$css_i.' a:hover {
						-webkit-border-radius: '.$r.'px '.$r.'px 0 0;
						-moz-border-radius: '.$r.'px '.$r.'px 0 0;
						border-radius: '.$r.'px '.$r.'px 0 0;
					}
					'.$css_c.'.rounded div.'.$this->alias.'_content  {
			...
			if ($this->params->line_color != '#B4B4B4') {
				$style .= '
					div.'.$this->alias.'_nav '.$css_i.' a,
					div.'.$this->alias.'_nav '.$css_i.' a:hover,
					div.'.$this->alias.'_content {
			...
			foreach ($sets as $items) {
				...
				foreach ($items as $i => $item) {
					...
					if (preg_match($s, $str, $match)) {
						...
						if ($i == $first) {
							if (!$script_set) {
								$html[] = '<script type="text/javascript">document.write( '
									...
									.'div.'.$this->alias.'_item_inactive { height: 0; }'
									.'div.'.$this->alias.'_nav { display: block !important; }'
									.'div.'.$this->alias.'_nav li.'.$this->alias.'_count_'.$item->count.' { display: block !important; }'
							...
							$html[] = '<div class="'.$this->alias.'_content" id="'.$this->alias.'_content_'.$item->set.'">';
						} else {
						...
						$html[] = '<div class="'.trim($this->alias.'_item '.$this->alias.'_count_'.$item->count.' '.trim($item->class)).' '.$this->alias.'_item_'.($item->active ? '' : 'in').'active" id="'.$this->alias.'_item_'.$item->id.'">';
	...
	function getNav(&$items)
	{
		...
		$html[] = '<div class="'.$this->alias.'_nav" style="display:none;">';
		$html[] = '<ul class="'.$this->alias.'_'.$this->item.'s">';
		foreach ($items as $item) {
			...
			$html[] = '<li style="display:none;" class="'.trim($this->alias.'_'.$this->item.' '.$this->alias.'_count_'.$item->count.' '.($item->active ? ' active' : '').' '.trim($item->class)).'" id="'.$this->alias.'_'.$this->item.'_'.$item->id.'"><span class="'.$this->alias.'_alias_'.$item->alias.'">'.$link.'</span></li>';
		}
		$html[] = '</ul>';
		$html[] = '<div style="clear:both;"></div>';
		$html[] = '</div>';
